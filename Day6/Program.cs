...
namespace Day6
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var coordinateWithMaxFiniteTerritory = coordinateTerritories.OrderByDescending(kv => kv.Value).Where(kv => !coordinatesWithInfiniteSpace.Contains(kv.Key)).First();

            Console.WriteLine($"Coordinate with max finite territory: ({coordinateWithMaxFiniteTerritory.Key.X},{coordinateWithMaxFiniteTerritory.Key.Y}) with territory {coordinateWithMaxFiniteTerritory.Value}");
        ...
        static IDictionary<Coordinate, int> CoordinateTerritoriesInBoundingRectangle(List<Coordinate> coordinates, Rectangle boundingRectangle)
        {
            var sw = Stopwatch.StartNew();
            var coordinatesWithInfiniteSpace = new HashSet<Coordinate>();
            ...
            for (int x = boundingRectangle.Left; x <= boundingRectangle.Right; x++)
            {
                for (int y = boundingRectangle.Top; y <= boundingRectangle.Bottom; y++)
                {
                    ...
                    foreach (var coordinate in coordinates)
                    {
                        if (coordinate.DistanceTo(x, y) < minDistance)
                        {
                            minDistance = coordinate.DistanceTo(x, y);
                            closestCoordinate = coordinate;
                        }
                    }
